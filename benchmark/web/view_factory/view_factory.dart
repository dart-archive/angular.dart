...
class ViewFactoryInvocaton {
  ...
  List<Node> _getElements(String template) {
    var div = new DivElement()
      ..setInnerHtml(template, treeSanitizer: new NullTreeSanitizer());
...
final TEMPLATE_TEXT_WITH_NG_BINDING =
    '<span><span ng-class="{}">{{1 + 2}}</span>'
    '<span ng-if="1 != 2">left</span>'
    '<span ng-if="1 != 2">right</span>'
    '</span>';
...
final TEMPLATE_COMPONENT_NO_CSS =
    '<component-without-css></component-without-css>';

final TEMPLATE_COMPONENT_WITH_CSS = '<component-with-css></component-with-css>';

final TEMPLATE_CONTAINER_COMPONENT =
    '<component-with-emulated></component-with-emulated>';

@Component(
    selector: 'component-without-css', template: 'empty', useShadowDom: true)
...
@Component(
    ...
    '<emulated-with-css></emulated-with-css>', useShadowDom: true)
...
void main() {
  ...
  js.context['benchmarkSteps'].add(new js.JsObject.jsify({
    "name": "ViewFactory.call",
    "fn": new js.JsFunction.withThis(viewFactory)
  }));
}
