...
class Reporter {
  ...
  bool get _requireCredentials =>
      Platform.environment["TRAVIS_SECURE_ENV_VARS"] == "true";
  ...
  Reporter() {
    ...
    if (user_email != null &&
        ...
        user_secret != null &&
        user_secret.isNotEmpty) {
      cookies = [
        new Cookie("user_email", user_email),
  ...
  _sendNextMessage() {
    ...
    if (reportId == null) {
      new HttpClient()
          .postUrl(Uri.parse("${baseUrl}/api/run"))
          .then(onRequest)
      ...
      new HttpClient()
          .putUrl(Uri.parse("${baseUrl}/api/run/id=$reportId"))
          .then(onRequest)
          .then((HttpClientResponse response) {
  ...
  toJson() =>
      {"commit_sha": commitSha, "tree_sha": treeSha, "data": data.toJson(),};
}
