...
class ElementBinderFactory {
  ...
  ElementBinderFactory(this._parser, this._perf, this._config, this._expando,
      this.astParser, this.componentFactory, this.shadowDomComponentFactory,
  ...
  ElementBinder binder(ElementBinderBuilder b) => new ElementBinder(_perf,
      _expando, _parser, _config, b._injector, b.componentData, b.decorators,
      b.onEvents, b.bindAttrs, b.childMode);
...
class ElementBinderBuilder {
  ...
  ElementBinderBuilder(
      this._factory, this._formatters, this._directives, this._injector);
  ...
  addDirective(DirectiveRef ref) {
    ...
    if (annotation.children == Directive.TRANSCLUDE_CHILDREN) {
      ...
      componentData = new BoundComponentData(
          ref, () => factory.bind(ref, _directives, _injector));
    } else {
      decorators.add(ref);
    }
    ...
    if (annotation.map != null) {
      annotation.map.forEach((attrName, mapping) {
        ...
        if (mode != '@' && mode != '&') {
          var value = attrName == "."
              ? ref.value
              : (ref.element as dom.Element).attributes[attrName];
        ...
        ref.mappings
            .add(new MappingParts(attrName, ast, mode, dstAST, mapping));
      });
    }
  }
  ...
  ElementBinder get binder {
    ...
    return template == null
        ? elBinder
        : _factory.templateBinder(this, elBinder);
  }
}
