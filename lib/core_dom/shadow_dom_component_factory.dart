...
abstract class BoundComponentFactory {
  ...
  Factory call(dom.Element element);
...
class BoundShadowDomComponentFactory implements BoundComponentFactory {
  ...
  static final _CALL_ARGS = [DIRECTIVE_INJECTOR_KEY, INJECTOR_KEY, SCOPE_KEY, NG_BASE_CSS_KEY,
                             EVENT_HANDLER_KEY];
  Function call(dom.Element element) {
    return (DirectiveInjector injector, Injector appInjector, Scope scope, NgBaseCss baseCss,
      ...
      var shadowScope = scope.createChild({'SHADOW': true}); // Isolate
              ...
              return _viewFuture.then((ViewFactory viewFactory) {
                if (shadowScope.isAttached) {
                  shadowDom.nodes.addAll(
                      viewFactory.call(shadowInjector.scope, shadowInjector, appInjector).nodes);
                }
                return shadowDom;
              });
            }
      ...
      var probe;
      shadowInjector = new ShadowDomComponentDirectiveInjector(injector, appInjector, shadowScope,
          templateLoader, shadowDom);
