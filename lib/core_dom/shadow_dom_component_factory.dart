...
abstract class BoundComponentFactory {
  ...
  static async.Future<ViewFactory> _viewFactoryFuture(Component component,
      ViewFactoryCache viewCache, DirectiveMap directives,
      TypeToUriMapper uriMapper, ResourceUrlResolver resourceResolver,
      Type type) {
    if (component.template != null) {
      ...
      return new async.Future.value(
          viewCache.fromHtml(component.template, directives, baseUri));
    }
  ...
  static void _setupOnShadowDomAttach(
      component, TemplateLoader templateLoader, Scope shadowScope) {
...
class ShadowDomComponentFactory implements ComponentFactory {
  ...
  ShadowDomComponentFactory(this.viewCache, this.platformShim, this.expando,
      this.config, this.uriMapper, this.resourceResolver, Http http,
    ...
    cacheRegister.registerCache(
        "ShadowDomComponentFactoryStyles", styleElementCache);
...
class BoundShadowDomComponentFactory implements BoundComponentFactory {
  ...
  BoundShadowDomComponentFactory(this._componentFactory, this._ref,
    ...
    _styleElementsFuture = _componentFactory
    ...
    final viewFactoryCache = new ShimmingViewFactoryCache(
        _componentFactory.viewCache, _tag, _componentFactory.platformShim);

    _shadowViewFactoryFuture = BoundComponentFactory._viewFactoryFuture(
        _component, viewFactoryCache, directives, _componentFactory.uriMapper,
        _componentFactory.resourceResolver, _ref.type);

    if (_shadowViewFactoryFuture != null) {
      _shadowViewFactoryFuture
          .then((viewFactory) => _shadowViewFactory = viewFactory);
    }
  }

  List<Key> get callArgs => _CALL_ARGS;
  static final _CALL_ARGS = [
  ...
  Function call(dom.Element element) {
    return (DirectiveInjector injector, Scope scope, View view,
        NgBaseCss baseCss, ShadowBoundary parentShadowBoundary) {
      ...
      try {
        ...
        var probe;
        var eventHandler = new ShadowRootEventHandler(shadowRoot,
            injector.getByKey(EXPANDO_KEY),
            injector.getByKey(EXCEPTION_HANDLER_KEY));
        final shadowInjector = new ComponentDirectiveInjector(injector,
            _injector, eventHandler, null, templateLoader, shadowRoot, null,
            _ref.typeKey, view, shadowBoundary);
        ...
        if (_component.useNgBaseCss && baseCss.urls.isNotEmpty) {
          if (baseCss.styles == null) {
            final f = _componentFactory
                .cssLoader(_tag, baseCss.urls)
                .then((cssList) {
        ...
        if (_shadowViewFactoryFuture != null) {
          if (_shadowViewFactory == null) {
            final f = _shadowViewFactoryFuture.then(
                (ViewFactory viewFactory) => _insertView(viewFactory,
                    shadowRoot, shadowInjector.scope, shadowInjector));
            futures.add(f);
          } else {
            final f = new Future.microtask(() {
              _insertView(_shadowViewFactory, shadowRoot, shadowInjector.scope,
                  shadowInjector);
            });
            futures.add(f);
          }
        ...
        if (_componentFactory.config.elementProbeEnabled) {
          ElementProbe probe = _componentFactory.expando[shadowRoot] =
              shadowInjector.elementProbe;
          shadowScope.on(ScopeEvent.DESTROY).listen(
              (ScopeEvent) => _componentFactory.expando[shadowRoot] = null);
        }
  ...
  dom.Node _insertView(ViewFactory viewFactory, dom.ShadowRoot shadowRoot,
      Scope shadowScope, ComponentDirectiveInjector shadowInjector) {
    if (shadowScope.isAttached) {
      shadowRoot.nodes
          .addAll(viewFactory.call(shadowInjector.scope, shadowInjector).nodes);
    }
  }
}
