...
class DirectiveSelector {
  ...
  DirectiveSelector(this._directives, this._formatters, this._binderFactory,
      this._interpolate, this._astParser, this._injector) {
    _directives.forEach((Directive annotation, Type type) {
      ...
      if ((match = _CONTAINS_REGEXP.firstMatch(selector)) != null) {
        ...
        elementSelector.addDirective(
            selectorParts, new _Directive(type, annotation));
      } else {
  ...
  ElementBinder matchElement(dom.Node node) {
    ...
    ElementBinderBuilder builder =
        _binderFactory.builder(_formatters, _directives, _injector);
    ...
    // Select node
    partialSelection = elementSelector.selectNode(
        builder, partialSelection, element, nodeName);

    // Select .name
    for (var name in element.classes) {
      classes.add(name);
      partialSelection =
          elementSelector.selectClass(builder, partialSelection, element, name);
    }
    ...
    element.attributes.forEach((attrName, value) {
      ...
      for (var k = 0; k < attrSelector.length; k++) {
        ...
        if (selectorRegExp.regexp.hasMatch(value)) {
          ...
          _directives[selectorRegExp.selector]
              .forEach((DirectiveTypeTuple tuple) {
            // Pre-compute the AST to watch this value.
            Interpolation interpolation = _interpolate(value);
            AST valueAST =
                _astParser(interpolation.expression, formatters: _formatters);
            builder.addDirective(new DirectiveRef(node, tuple.type,
                tuple.directive, new Key(tuple.type), attrName, valueAST,
                interpolation.bindingExpressions));
          });
        }
      }

      partialSelection = elementSelector.selectAttr(
          builder, partialSelection, node, attrName, value);
    });
    ...
    while (partialSelection != null) {
      ...
      elementSelectors.forEach((_ElementSelector elementSelector) {
        classes.forEach((className) {
          partialSelection = elementSelector.selectClass(
              builder, partialSelection, node, className);
        });
  ...
  ElementBinder matchComment(dom.Node node) =>
      _binderFactory.builder(null, null, _injector).binder;
}
...
class DirectiveSelectorFactory {
  ...
  DirectiveSelector selector(DirectiveMap directives,
      [Injector injector, FormatterMap formatters]) => new DirectiveSelector(
      directives, formatters != null ? formatters : _defaultFormatterMap,
...
class _ContainsSelector {
  ...
  _ContainsSelector(this.selector, String regexp) : regexp = new RegExp(regexp);
}

final _SELECTOR_REGEXP = new RegExp(r'^(?:([-\w]+)|' // "tag"
    r'(?:\.([-\w]+))|' // ".class"
    r'(?:\[([-\w*]+)(?:=([^\]]*))?\]))'); // "[name]", "[name=value]" or "[name*=value]"
final _CONTAINS_REGEXP =
    new RegExp(r'^:contains\(\/(.+)\/\)$'); // ":contains(/text/)"
...
_addRefs(
    ElementBinderBuilder builder, List<_Directive> directives, dom.Node node,
    [String attrValue]) {
  directives.forEach((directive) {
    builder.addDirective(new DirectiveRef(node, directive.type,
        directive.annotation, new Key(directive.type), attrValue));
  });
}
...
class _ElementSelector {
  ...
  final _attrValuePartialMap =
      new HashMap<String, Map<String, _ElementSelector>>();
    ...
    for (var i = 0; i < selectorParts.length; i++) {
      ...
      if ((name = part.element) != null) {
        if (terminal) {
          ...
          elSelector = elSelector._elementPartialMap.putIfAbsent(
              name, () => new _ElementSelector(name));
        }
      } else if ((name = part.className) != null) {
        if (terminal) {
          elSelector._classMap.putIfAbsent(name, () => []).add(directive);
        } else {
          elSelector = elSelector._classPartialMap.putIfAbsent(
              name, () => new _ElementSelector(name));
        }
      } else if ((name = part.attrName) != null) {
        if (terminal) {
          elSelector._attrValueMap
              .putIfAbsent(name, () => new HashMap<String, List<_Directive>>())
  ...
  List<_ElementSelector> selectNode(ElementBinderBuilder builder,
      List<_ElementSelector> partialSelection, dom.Node node, String nodeName) {
  ...
  List<_ElementSelector> selectClass(ElementBinderBuilder builder,
      List<_ElementSelector> partialSelection, dom.Node node,
      String className) {
  ...
  List<_ElementSelector> selectAttr(ElementBinderBuilder builder,
      List<_ElementSelector> partialSelection, dom.Node node, String attrName,
      String attrValue) {
  ...
  String _matchingKey(Iterable<String> keys, String attrName) => keys
      .firstWhere((key) => _matchingKeyCache
          .putIfAbsent(
              key, () => new RegExp('^${key.replaceAll('*', r'[-\w]+')}\$'))
