...
@Decorator(selector: 'input[type=week][ng-model]',
    module: InputDateLike.moduleFactory)
class InputDateLike {
  static void moduleFactory(DirectiveModule module)
        => module.bind(NgBindTypeForDateLike,
            toFactory: (p) => new NgBindTypeForDateLike(p[0]), inject: [ELEMENT_KEY]);
  final dom.InputElement inputElement;
  final NgModel ngModel;
  final NgModelOptions ngModelOptions;
  final Scope scope;
...
class NgValue {
  static module(DirectiveModule module) => module.bind(NgValue, visibility: Visibility.LOCAL);
