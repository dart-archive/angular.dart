...
class Filter extends syntax.Filter {
  ...
  eval(scope, FilterMap filters) => Function.apply(filters(name), evalList(scope, allArguments, filters));
}

class Assign extends syntax.Assign {
  Assign(syntax.Expression target, value) : super(target, value);
  eval(scope, FilterMap filters) => target.assign(scope, value.eval(scope, filters), filters);
}
...
class LiteralArray extends syntax.LiteralArray {
  ...
  eval(scope, FilterMap filters) => elements.map((e) => e.eval(scope, filters)).toList();
}
...
class LiteralObject extends syntax.LiteralObject {
  ...
  eval(scope, FilterMap filters) => new Map.fromIterables(keys, values.map((e) => e.eval(scope, filters)));
}
