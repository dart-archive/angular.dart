 ...
 * defined message template code to be reused and overridden. Upon compile, NgMessages
 * renders inner [NgMessage] messages based on the state of the given key/value map.
 * If the value property within the provided map is a truthy value then the first inner
 * element containing a [NgMessage] directive that matches the key of that truthy
 * value will be inserted into the DOM. By default, only one NgMessages element (the first detected
 * instance) will be displayed at a time, however, this behavior can be overridden by
 * providing the ng-messages-multiple attribute on the element (with a truthy value).
 * 
