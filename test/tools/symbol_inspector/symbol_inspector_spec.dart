...
Symbol symbolForName(libraryInfo, String name) => libraryInfo.names
    .firstWhere((x) => unwrapSymbol(x.qualified) == name).qualified;
...
void main() => describe('symbol inspector', () {
  ...
  it('should extract all used symbols for a class', () {
    ...
    expect(libraryInfo.symbolsUsedForName[
            symbolForName(libraryInfo, 'simple_library.A')].map(unwrapSymbol),
        unorderedEquals([
      'simple_library.A',
      'simple_library.FieldType',
      'simple_library.StaticFieldType',
  ...
  it('should extract all used symbols for a typedef', () {
    ...
    expect(libraryInfo.symbolsUsedForName[
        symbolForName(libraryInfo, 'simple_library.TypedefType')]
  ...
  it('should not extract generic types', () {
    ...
    expect(libraryInfo.symbolsUsedForName[
        symbolForName(libraryInfo, 'simple_library.Generic')]
        .map(unwrapSymbol), unorderedEquals(['simple_library.Generic']));
  });
  ...
  describe('assert', () {
    ...
    it('should assert symbol names are correct', () {
      assertSymbolNamesAreOk(
          SIMPLE_LIBRARY_SYMBOLS, getSymbolsFromLibrary(("simple_library")));
    });

    it('should throw if the list is missing symbols', () {
      expect(() => assertSymbolNamesAreOk(
          [], getSymbolsFromLibrary(("simple_library"))), throwsA(contains(
              "These symbols are exported thru the angular "
