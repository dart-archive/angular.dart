...
void main() {
  describe('html_extractor', () {
    it('should extract text mustache expressions', () {
      ...
      expect(extractor.expressions.toList()..sort(), equals(['aux', 'bar']));
    });
    ...
    it('should extract ng-repeat expressions', () {
      ...
      var extractor = new HtmlExpressionExtractor([]);
      extractor.crawl('/', ioService);
      expect(extractor.expressions.toList()..sort(), equals(['bar']));
    });
    ...
    it('should extract expressions provided in the directive info', () {
      ...
      extractor.crawl('/', ioService);
      expect(extractor.expressions.toList()..sort(), equals(['bar', 'foo']));
    });

    it('should extract expressions from expression attributes', () {
      var ioService = new MockIoService({'foo.html': r'<foo bar="baz"></foo>'});

      var extractor =
          new HtmlExpressionExtractor([new DirectiveInfo('foo', ['bar'])]);
      extractor.crawl('/', ioService);
    ...
    it('should extract expressions from expression attributes for camelCased attributes',
        () {
      var ioService =
          new MockIoService({'foo.html': r'<foo fooBar="baz"></foo>'});

      var extractor =
          new HtmlExpressionExtractor([new DirectiveInfo('foo', ['fooBar'])]);
      extractor.crawl('/', ioService);
      expect(extractor.expressions.toList()).toEqual(['baz']);
    });
