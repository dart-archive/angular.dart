...
main() {
  describe('PlatformJsBasedShim', () {
    ...
    it('should scope styles to shadow dom across browsers.', async((TestBed _,
        MockHttpBackend backend, PlatformJsBasedShim platform) {
      ...
      Element element =
          e('<span><test-wptc><span>ignore</span></test-wptc></span>');

      _.compile(element);

      backend
        ..flushGET('${TEST_SERVER_BASE_PREFIX}test/core_dom/style.css').respond(
            200, 'span {background-color: red;}')
      ...
      try {
        ...
        expect(element.getComputedStyle().backgroundColor).not
            .toEqual("rgb(255, 0, 0)");

        // Inner span should not be styled.
        expect(element.children[0].children[0]
    ...
    it('should scope :host styles to the primary element.', async((TestBed _,
        MockHttpBackend backend, PlatformJsBasedShim platform) {
    ...
    xit('should scope ::content rules to component content.',
      ...
      backend
        ...
        ..flushGET('template.html').respond(
            200, '<span><content></content></span>');
    ...
    xit('should style into child shadow dom with ::shadow.',
      ...
      backend
        ..flushGET('outer-style.css').respond(
            200, 'my-inner::shadow .foo {background-color: red; }')
        ..flushGET('outer-html.html').respond(
            200, '<my-inner><span class="foo">foo</span></my-inner>');

      microLeap();

      backend
        ..flushGET('inner-style.css').respond(200, '/* no style */')
        ..flushGET('inner-html.html').respond(
            200, '<span class="foo"><content></content></span>');
...
@Component(
    selector: "test-wptc", templateUrl: "template.html", cssUrl: "style.css")
