...
const _aBElementDirectiveValue =
    const Decorator(selector: 'b[directive=value]');
...
const _aStructural = const Decorator(
    selector: '[structural]', children: Directive.TRANSCLUDE_CHILDREN);
const _aIgnoreChildren = const Decorator(
    selector: '[ignore-children]', children: Directive.IGNORE_CHILDREN);
...
@_aWildcardDirectiveAttr
class _WildcardDirectiveAttr {}
@_aDirectiveFooAttr
class _DirectiveFooAttr {}
@_aBElementDirectiveAttr
class _BElementDirectiveAttr {}
@_aDirectiveValueAttr
class _DirectiveValueAttr {}
@_aBElementDirectiveValue
class _BElementDirectiveValue {}
@_aContainsAbc
class _ContainsAbc {}
@_aAttributeContainsXyz
class _AttributeContainsXyz {}
@_aCComponent
class _CComponent {}
@_aAttribute
class _Attribute {}
...
@_aOneOfTwoDirectives
class _OneOfTwoDirectives {}
@_aTwoOfTwoDirectives
class _TwoOfTwoDirectives {}
  ...
  describe('Selector', () {
    ...
    describe('matchElement', () {
      ...
      it('should match directive on element', () {
        expect(selector(element = e('<b></b>')), toEqualsDirectiveInfos([
          {
            "selector": 'b',
            "value": null,
            "element": element,
            "annotation": _aBElement
          }
        ]));
      });
      ...
      it('should match directive on element[attribute]', () {
        expect(selector(element = e('<b directive=abc></b>')),
          ...
          {
            "selector": 'b[directive]',
            "value": 'abc',
            "element": element,
            "annotation": _aBElementDirectiveAttr
          }
        ]));
      });
      ...
      it('should match directive on [attribute=value]', () {
        expect(selector(element = e('<div directive=value></div>')),
          ...
          {
            "selector": '[directive=value]',
            "value": 'value',
            "element": element,
            "annotation": _aDirectiveValueAttr
          }
        ]));
      });
      ...
      it('should match directive on element[attribute=value]', () {
        expect(selector(element = e('<b directive=value></div>')),
          ...
          {
            "selector": 'b[directive=value]',
            "value": 'value',
            "element": element,
            "annotation": _aBElementDirectiveValue
          }
        ]));
      });
      ...
      it('should match attribute names', () {
        expect(selector(element = e('<div wildcard-match=ignored></div>')),
            toEqualsDirectiveInfos([
          {
            ...
            "element": element,
            "name": 'wildcard-match',
            "annotation": _aWildcardDirectiveAttr
          }
        ]));
      });

      it('should sort by priority', async(() {
        TemplateElementBinder eb = selector(element =
            e('<component attribute ignore-children structural></component>'));
        expect(eb, toEqualsDirectiveInfos(null,
            template: {
          "selector": "[structural]",
          "value": "",
          "element": element,
          "annotation": _aStructural
        }));

        expect(eb.templateBinder, toEqualsDirectiveInfos([
          {
            "selector": "[attribute]",
            "value": "",
            "element": element,
            "annotation": _aAttribute
          },
          {
            "selector": "[ignore-children]",
            "value": "",
            "element": element,
            "annotation": _aIgnoreChildren
          }
        ],
            component: {
          "selector": "component",
      ...
      it('should match on multiple directives', () {
        expect(selector(element = e('<div directive="d" foo="f"></div>')),
          ...
          {
            "selector": '[directive=d][foo=f]',
            "value": 'f',
            "element": element,
            "annotation": _aDirectiveFooAttr
          }
        ]));
      });
      ...
      it('should match ng-model + required on the same element', () {
        expect(selector(element = e(
                '<input type="text" ng-model="val" probe="i" required="true" />')),
            toEqualsDirectiveInfos([
          ...
          {
            "selector": 'input[type=text][ng-model]',
            "value": 'val',
            "element": element
          }
        ]));
      ...
      it('should match two directives', () {
        expect(selector(element =
                e('<input type="text" my-model="val" required my-required />')),
            toEqualsDirectiveInfos([
          {"selector": '[my-model][required]', "value": '', "element": element},
          {
            "selector": '[my-model][my-required]',
            "value": '',
            "element": element
          }
        ]));
      ...
      it('should match an two directives with the same selector', () {
        expect(selector(element = e('<div two-directives></div>')),
            toEqualsDirectiveInfos([
          {
            "selector": '[two-directives]',
            "value": '',
            "element": element,
            "annotation": _aOneOfTwoDirectives
          },
          {
            "selector": '[two-directives]',
            "value": '',
            "element": element,
            "annotation": _aTwoOfTwoDirectives
          }
        ]));
      });
      ...
      it('should collect on-* attributes', () {
        ElementBinder binder =
            selector(e('<input on-click="foo" on-blah="fad"></input>'));
        expect(binder.onEvents).toEqual({'on-click': 'foo', 'on-blah': 'fad'});
      });

      it('should collect bind-* attributes', () {
        ElementBinder binder =
            selector(e('<input bind-x="y" bind-z="yy"></input>'));
...
class DirectiveInfosMatcher extends Matcher {
  ...
  DirectiveInfosMatcher(this.expected,
      {this.expectedTemplate, this.expectedComponent}) {
    if (expected != null) {
      expected
          .sort((a, b) => Comparable.compare(safeToString(a), safeToString(b)));
    }
  }
  ...
  bool _refMatches(DirectiveRef directiveRef, Map expectedMap) =>
      directiveRef.element == expectedMap['element'] &&
          ...
          (expectedMap['annotation'] == null ||
              directiveRef.annotation == expectedMap['annotation']) &&
          (directiveRef.valueAST == null ||
              directiveRef.valueAST.expression == expectedMap['ast']);
  ...
  bool matches(ElementBinder binder, matchState) {
    ...
    if (expected != null) {
      var decorators = new List.from(binder.decorators)
        ..sort((a, b) =>
            Comparable.compare(safeToStringRef(a), safeToStringRef(b)));
    ...
    if (pass && expectedTemplate != null) {
      pass = pass &&
          _refMatches(
              (binder as TemplateElementBinder).template, expectedTemplate);
    }
...
Matcher toEqualsDirectiveInfos(List<Map> directives,
    {Map template, Map component}) => new DirectiveInfosMatcher(directives,
    expectedTemplate: template, expectedComponent: component);
