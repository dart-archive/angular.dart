...
main() {
  describe('NonBindableDirective', () {
    ...
    it('should set ignore all other markup/directives on the descendent nodes with data attribute',
      ...
      var element = $('<div>' +
                      '  <span id="s1">{{a}}</span>' +
                      '  <span id="s2" data-ng-bind="b"></span>' +
                      '  <div foo="{{a}}" data-ng-non-bindable>' +
                      '    <span data-ng-bind="a"></span>{{b}}' +
                      '  </div>' +
                      '  <span id="s3">{{a}}</span>' +
                      '  <span id="s4" data-ng-bind="b"></span>' +
                      '</div>');
      compiler(element, directives)(injector, element);
      scope.context['a'] = "one";
      scope.context['b'] = "two";
      ...
      var nonBindableDiv = element.find("div");
      expect(nonBindableDiv.html().trim()).toEqual('<span data-ng-bind="a"></span>{{b}}');
      expect(nonBindableDiv.text().trim()).toEqual('{{b}}');
      // Bindings on the same node are processed.
      expect(nonBindableDiv.attr('foo')).toEqual('one');
    }));
