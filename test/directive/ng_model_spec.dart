...
void main() {
  describe('ng-model', () {
    ...
    describe('type="color"', () {
      ...
      it('should update model from the input value', () {
        var inputEl = _.compile('<input type="color" ng-model="model">');
        var ngModel = ngInjector('input', inputEl).get(NgModel);

        inputEl.value = '#000000';
        _.triggerEvent(inputEl, 'change');
        expect(_.rootScope.context.model).toEqual('#000000');

        inputEl.value = '#ffffff';
        var input = ngInjector('input', inputEl).get(InputTextLike);
        input.processValue();
      ...
      it('should only render the input value upon the next digest', (Scope scope) {
        ...
        expect(inputEl.value).not.toEqual('#aabbcc');

        scope.apply();
        expect(inputEl.value).toEqual('#aabbcc');
      });
    });
