...
main() {
  describe('ngBindRoute', () {
    ...
    it('should inject null RouteProvider when no ng-bind-route', async(() {
      ...
      expect(_.rootScope.context.$probes['routeProbe'].injector.get(RouteProvider)).toBeNull();
    }));
    ...
    it('should inject RouteProvider with correct flat route', async(() {
      ...
      expect(_.rootScope.context.$probes['routeProbe'].injector.get(RouteProvider).routeName)
          .toEqual('library');
    }));
    ...
    it('should inject RouteProvider with correct nested route', async(() {
      Element root = _.compile(
          ...
          '  </div>'
          '</div>');
      expect(_.rootScope.context.$probes['routeProbe'].injector.get(RouteProvider).route.name)
          .toEqual('all');
    }));
    ...
    it('should expose NgBindRoute as RouteProvider', async(() {
      Element root = _.compile(
          '<div ng-bind-route="library"><div probe="routeProbe"></div></div>');
      expect(_.rootScope.context.$probes['routeProbe'].injector.get(RouteProvider) is NgBindRoute).toBeTruthy();
    }));
