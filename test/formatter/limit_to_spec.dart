...
main() {
  describe('orderBy formatter', () {
    ...
    it('should return the first X items when X is positive',
      ...
      expect(parse('list | limitTo: 3').eval(scope.context, formatters))
          .toEqual(['a', 'b', 'c']);
      expect(parse('list | limitTo: limit').eval(scope.context, formatters))
          .toEqual(['a', 'b', 'c']);
      expect(parse('string | limitTo: 3').eval(scope.context, formatters))
          .toEqual('tuv');
      expect(parse('string | limitTo: limit').eval(scope.context, formatters))
          .toEqual('tuv');
    });
    ...
    it('should return an null when limiting null list',
        (Scope scope, Parser parse, FormatterMap formatters) {
      expect(parse('null | limitTo: 1').eval(scope.context, formatters))
          .toEqual(null);
      expect(parse('thisIsNull | limitTo: 1').eval(scope.context, formatters))
          .toEqual(null);
    });

    it('should return an empty array when X cannot be parsed',
        (Scope scope, Parser parse, FormatterMap formatters) {
      expect(parse('list | limitTo: bogus').eval(scope.context, formatters))
          .toEqual([]);
      expect(parse('string | limitTo: null').eval(scope.context, formatters))
          .toEqual([]);
      expect(parse('string | limitTo: thisIsNull').eval(
    ...
    it('should return a copy of input array if X is exceeds array length',
      ...
      expect(parse('list | limitTo: -20').eval(scope.context, formatters))
          .toEqual(scope.context['list']);
      expect(parse('list | limitTo: 20').eval(scope.context, formatters)).not
          .toBe(scope.context['list']);
    });
