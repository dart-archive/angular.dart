...
void main() {
  describe('MockHttpBackend', () {
    ...
    it('should throw error when unexpected request', () {
      ...
      expect(() {
        ...
        hb.flush();
          ...
          message: 'Unexpected request: GET /xxx\nNo more requests expected');
    });
    ...
    it('should preserve the order of requests', () {
      ...
      expect(callback.calls[0].positionalArguments)
          .toEqual([201, 'second', '']);
    ...
    describe('respond()', () {
      ...
      it('should take function', () {
        ...
        expect(callback).toHaveBeenCalledOnceWith(
            301, 'GET/some;data;a=b', 'Connection: keep-alive');
      });
      ...
      it('should default response headers to ""', () {
        ...
        expect(callback.calls[0].positionalArguments)
            .toEqual([200, 'first', '']);
        expect(callback.calls[1].positionalArguments)
            .toEqual([200, 'second', '']);
      });
    });
    ...
    describe('expect()', () {
      it('should require specified order', () {
        expect(() {
          ...
          hb.flushExpected('GET', '/url1').respond(200, '');
            ...
            message: 'Unexpected request: GET /url2\nExpected GET /url1');
      });
      ...
      it('should throw exception when only headers differs from expectation',
        ...
        hb
            .expect('GET', '/match', null, {'Content-Type': 'application/json'})
            .respond(200, '', {});
        ...
        expect(() {
          ...
          hb.flush();
            ...
            message: 'Expected GET /match with different headers\n' +
      ...
      it('should throw exception when only data differs from expectation', () {
        ...
        expect(() {
          ...
          hb.flush();
            ...
            message: 'Expected GET /match with different data\n' +
                'EXPECTED: some-data\nGOT:      different');
      });
    ...
    describe('flush()', () {
      ...
      it('should throw exception when flushing more requests than pending', () {
        ...
        expect(() {
          hb.flush(2);
        }).toThrowWith(message: 'No more pending request to flush !');
        expect(callback).toHaveBeenCalledOnce();
      });
      ...
      it('should throw exception when no request to flush', () {
        expect(() {
          hb.flush();
        }).toThrowWith(message: 'No pending request to flush !');
      ...
      it('should throw exception if not all expectations satisfied', () {
        ...
        expect(() {
          hb.flush();
        }).toThrowWith(message: 'Unsatisfied requests: GET /url2');
      });
    });
    ...
    it('should abort requests when timeout promise resolves', () {
      ...
      var canceler,
          then = guinness.createSpy('then').andCallFake((fn) {
            canceler = fn;
          });
    ...
    describe('MockHttpExpectation', () {
      it('should accept url as regexp', () {
        ...
        expect(exp.match(new RecordedRequest(method: 'GET', url: '/x')))
            .toBe(true);
        expect(exp.match(new RecordedRequest(method: 'GET', url: '/xxx/x')))
            .toBe(true);
        expect(exp.match(new RecordedRequest(method: 'GET', url: 'x')))
            .toBe(false);
        expect(exp.match(new RecordedRequest(method: 'GET', url: 'a/x')))
            .toBe(false);
      });

      it('should accept data as regexp', () {
        var exp =
            new MockHttpExpectation('POST', '/url', new RegExp('\{.*?\}'));

        expect(exp.match(new RecordedRequest(
            method: 'POST', url: '/url', data: '{"a": "aa"}'))).toBe(true);
        expect(exp.match(new RecordedRequest(
            method: 'POST', url: '/url', data: '{"one": "two"}'))).toBe(true);
        expect(exp.match(new RecordedRequest(
            method: 'POST', url: '/url', data: '{"one"'))).toBe(false);
      });
