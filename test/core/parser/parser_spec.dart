  ...
  describe('parse', () {
    ...
    describe('error handling', () {
      ...
      it('should throw on missing expected token', () {
        expectEval("a(b").toThrowWith(message: 'Parser Error: Missing expected ) the end of the '
                                               'expression [a(b]');
      });


      it('should throw on bad assignment', () {
        expectEval("5=4").toThrowWith(message: 'Parser Error: Expression 5 is not assignable at '
                                               'column 2 in [5=4]');
        expectEval("array[5=4]").toThrowWith(message: 'Parser Error: Expression 5 is not assignable'
                                                      ' at column 8 in [array[5=4]]');
      });


      it('should throw on incorrect ternary operator syntax', () {
        expectEval("true?1").toThrowWith(message: 'Parser Error: Conditional expression true?1 '
      ...
      it("should throw on an unexpected token", (){
        expectEval("[1,2] trac").toThrowWith(message: 'Parser Error: \'trac\' is an unexpected '
                                                      'token at column 7 in [[1,2] trac]');
      });
      ...
      it('should only allow identifier, string, or keyword as object literal key', () {
        expect(() => parser('{(:0}'))
            .toThrowWith(message: 'expected identifier, keyword, or string');
        expect(() => parser('{1234:0}'))
            .toThrowWith(message: 'expected identifier, keyword, or string');
      });
    });
    ...
    describe('setters', () {
      ...
      xit('should throw a nice error for type mismatch', () {
        ...
        }).toThrowWith(message: "Eval Error: Caught type 'String' is not a subtype of type 'int' "
                                "of 'value'. while evaling [obj.integer = \"hello\"]");
      });
    });
    ...
    describe('named arguments', () {
      ...
      it('should be an error to use the same name twice', () {
        expect(() => parser('foo(a: 0, a: 1)'))
            .toThrowWith(message: "Duplicate argument named 'a' at column 11");
        expect(() => parser('foo(a: 0, b: 1, a: 2)'))
            .toThrowWith(message: "Duplicate argument named 'a' at column 17");
        expect(() => parser('foo(0, a: 1, a: 2)'))
            .toThrowWith(message: "Duplicate argument named 'a' at column 14");
        expect(() => parser('foo(0, a: 1, b: 2, a: 3)'))
            .toThrowWith(message: "Duplicate argument named 'a' at column 20");
      });


      it('should be an error to use Dart reserved words as names', () {
        expect(() => parser('foo(if: 0)'))
            .toThrowWith(message: "Cannot use Dart reserved word 'if' as named argument at column 5");
        expect(() => parser('foo(a: 0, class: 0)'))
            .toThrowWith(message: "Cannot use Dart reserved word 'class' as named argument at column 11");
      });
    ...
    describe('formatters', () {
      ...
      it('should not allow formatters in a chain', () {
        ...
        }).toThrowWith(message: 'Cannot have a formatter in a chain the end of the expression '
                                '[1;\'World\'|hello]');
        expect(() {
          parser("'World'|hello;1");
        }).toThrowWith(message: 'Cannot have a formatter in a chain at column 15 in '
                                '[\'World\'|hello;1]');
      });
    });
  });
