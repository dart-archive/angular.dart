...
main() {
  describe('orderBy filter', () {
    ...
    it('should pass through null list when input list is null', (Scope scope, Parser parse, FilterMap filters) {
      var list = null;
      expect(parse('list | orderBy:"foo"').eval(scope.context, filters)).toBe(null);
    });

    it('should pass through argument when expression is null', (Scope scope, Parser parse, FilterMap filters) {
      var list = scope.context['list'] = [1, 3, 2];
      expect(parse('list | orderBy:thisIsNull').eval(scope.context, filters)).toBe(list);
    });

    it('should sort with "empty" expression using default comparator', (Scope scope, Parser parse, FilterMap filters) {
    ...
    it('should sort by expression', (Scope scope, Parser parse, FilterMap filters) {
    ...
    it('should reverse order when passed the additional descending param', (Scope scope, Parser parse, FilterMap filters) {
    ...
    it('should reverse order when expression is prefixed with "-"', (Scope scope, Parser parse, FilterMap filters) {
      expect(parse('authors | orderBy:"-lastName"').eval(scope.context, filters)).toEqual([
        Oscar___Wilde,
        Mark____Twain,
        Emily___Bronte,
